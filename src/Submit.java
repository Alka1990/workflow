...
public class Submit extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    try {
      ...
      int k;
      for (j = 0; j < aligners.length; ++j) {
        String[] x2 = aligners[j].split("~");

        for (k = 0; k < x2.length; ++k) {
          alignerstable[j][k] = x2[k];
        }
      }
      ...
      for (k = 0; k < review_grid.length; ++k) {
      ...
      if (decision.equals("Save As Drafts") && flag.equals("abc")) {
        ...
        String query41 = "Insert into patient_details( USER_ID,initiated_by,initiated_Date,DECISION, STATUS, STAGE, NEXT_STAGE,prerequisite,case_number,registered_doctor,clinic_name,clinic_address,gp,city,patient_name,patient_age,type_of_account,referred_by,channel,corporate_account,kol,discount,no_of_aligners,starter_kit,scanning,pickup,dispatch,dispatch_to,advance_starter,advance_scanning, scanning_service,dispatch_date,person_dispatch,person_scan,bill_to,gst_number,payment_mode,wo_number,wo_date,created_date,no_of_stages,current_stage,no_of_aligners_u_a,no_of_aligners_u_p,no_of_aligners_l_a,no_of_aligners_l_p,attachment_applicable,dispatch_grid,data_grid,dispatch_date2,final_amount,status_grid,payment_grid,batch_number,work_date,next_work,next_work_date,case_review,refinement_package,draftflag,crmname,priority) values( '"
            ...
            + type_of_account + "', '" + referred_by + "', '" + channel + "', '" + corporate_account + "', '" + kol
            + "', '" + discount + "', '" + no_of_aligners + "', '" + starter_kit + "', '" + scanning + "', '" + pickup
            + "', '" + dispatch + "', '" + dispatch_to + "', '" + advance_starter + "', '" + advance_scanning + "', '"
            + scanning_service + "', '" + dispatch_date + "', '" + person_dispatch + "', '" + person_scan + "', '"
            + bill_to + "', '" + gst_number + "', '" + payment_mode + "', '" + wo_number + "', '" + wo_date
            + "',  sysdate(), '" + no_of_stages + "', '" + current_stage + "', '" + no_of_aligners_u_a + "', '"
            + no_of_aligners_u_p + "', '" + no_of_aligners_l_a + "', '" + no_of_aligners_l_p + "', '"
            + attachment_applicable + "', '" + dispatch_grid + "', '" + data_grid + "', '" + dispatch_date2 + "', '"
            + final_amount + "', '" + status_grid + "', '" + payment_grid + "', '" + batch_number + "', '" + work_date
            + "', '" + next_work + "', '" + next_work_date + "', '" + case_review + "', '" + refinement_package
            + "','N','" + user_id + "','5')";
        ...
        query41 = "Insert into decision_history(decision, Remarks, date_time, stage, UserId, caseid) values( '"
            + decision + "','" + remarks + "', sysdate(),'Intro','" + userid + "','" + case_id + "') ";
        insertStmnt1.executeUpdate(query41);
        String query2 = "";
        ...
        try {
          ...
          for (z = 0; z < profiles.length; ++z) {
            if (paymentTable[z][1] != null || paymentTable[z][2] != null) {
              query2 = "insert into payment_grid values('" + case_id + "','" + paymentTable[z][0] + "','"
                  + paymentTable[z][1] + "','" + paymentTable[z][2] + "',sysdate())";
          ...
          for (z = 0; z < review_grid.length; ++z) {
            if (reviewtable[z][1] != null || reviewtable[z][2] != null || reviewtable[z][3] != null) {
              query2 = "insert into reviewnumbergrid values('" + case_id + "','" + reviewtable[z][0] + "','"
                  + reviewtable[z][1] + "','" + reviewtable[z][2] + "','" + reviewtable[z][3] + "',sysdate())";
              insertStmnt.executeUpdate(query2);
            }
          }
          ...
          for (z = 0; z < aligners.length; ++z) {
            if (alignerstable[z][1] != null || alignerstable[z][2] != null || alignerstable[z][3] != null) {
              query2 = "insert into no_of_aligers_table values('" + alignerstable[z][0] + "','" + alignerstable[z][1]
                  + "','" + alignerstable[z][2] + "','" + alignerstable[z][3] + "','" + case_id + "',sysdate())";
              insertStmnt.executeUpdate(query2);
            }
          }
        } catch (Exception var85) {
          LOGGER.info("Error At Searchcase=" + var85.getMessage());
        }

        out.print("Case Number  id " + case_id + " Created Successfully");
      }
    } catch (Exception var86) {
      LOGGER.info("Error At Searchcase=" + var86.getMessage());
    }
